...
def plot_rts(df):
    ...
    ####
    #### Programming Problem 3: Use the pandas to create barchart of sorted
    
    df['side_name'] = np.choose(df['side'], ['left', 'right'])

    # Now create a pivot table (specifying values, index, and columns)
    x = df.pivot_table(values='rts', index='targ_ecc', columns='side_name')
    # And now plot that pivot table
    x.plot(kind='bar', grid=None)
    # And add plot details (title, legend, xlabel, and ylabel)
    plt.xlabel('Eccentricity (degrees visual angle)')
    plt.ylabel('Reaction Time (msec)')
    plt.grid(False)
    plt.title('Reaction Time by Eccentricity and sides')
    plt.legend(['', 'Left','Right'], loc=2)
    plt.ylim([0, 1200])
...
def get_ems(df, trial):
    ...
    ####    df['stimon'][14]
    number_of_points = df['em_time'][trial].size
    total_time = df['em_time'][trial][-1] - df['em_time'][trial][0]
    sampling_rate = number_of_points  / total_time # points per unit of time

    time_of_signal_appearance = df['stimon'][trial]
    time_of_response =df['response'][trial]

    N1 = int(time_of_signal_appearance * sampling_rate)
    N2 = int(time_of_response * sampling_rate)

    t = df['em_time'][trial][N1:N2]
    h = df['em_horiz'][trial][N1:N2]
    v = df['em_vert'][trial][N1:N2]
...
def plot_ems_and_target(df, trial):
    ...
    x_target = df['targ_x'][trial]
    y_target = df['targ_y'][trial]
    horizontal = np.ones(t.size) * x_target
    vertical = np.ones(t.size) * y_target
    # Can get information about target location here

    plt.figure()
    plt.plot(t, h, 'r', label='Horizontal')
    plt.plot(t, v, 'g', label='Vertical')
    plt.legend()
    plt.hold(True)
    plt.plot(t, horizontal, 'r')
    plt.plot(t, vertical, 'g')
    plt.ylim([-10, 10])
    plt.xlabel('Time (msec)')
    plt.ylabel('Position (Degree visual angle)')
...
def get_rate(spk_times, start, stop):
    ...
    # Number of spikes between start and stop
    aux1 = spk_times >= start
    aux2 = spk_times < stop
    number_of_spikes = np.sum(aux1 * aux2)
    rate = number_of_spikes * 1.0/(stop - start)
